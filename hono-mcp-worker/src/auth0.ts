    ...
    return createMiddleware<{ Bindings: Env; Variables: Variables }>(
        async (c, next) => {
            ...
            if (
                ...
                const { AUTH0_DOMAIN } = c.env;
                const metadata = new ProtectedResourceMetadataBuilder(
                    options.resourceServerUrl.toString(),
                    [`https://${AUTH0_DOMAIN}/`]
                )
    ...
    return createMiddleware<{ Bindings: Env; Variables: Variables }>(
        async (c, next) => {
            try {
                ...
                if (error instanceof InvalidRequestError || error instanceof InvalidTokenError) {
                    // Per the MCP Authorization spec, requests with no access token (or a
                    // malformed one) must be treated the same as requests with an invalid
                    // token: respond 401 with a WWW-Authenticate header that points the
                    // client at the Protected Resource Metadata document so it can discover
                    // how to obtain a token.
                    const errorCode =
                        error instanceof InvalidTokenError ? error.errorCode : "invalid_token";
                    const errorDescription =
                        error instanceof InvalidTokenError
                            ? error.message
                            : "No access token was provided. Include a Bearer token in the Authorization header.";
                    ...
                    const wwwAuthValue = resourceMetadataUrl
                        ...
                        : `Bearer error="${errorCode}", error_description="${errorDescription}"`;
