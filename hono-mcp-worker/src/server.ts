...
const MCP_SERVER_RESOURCE_NAME = "Hono MCP Server";
...
// OAuth Client ID Metadata Document (CIMD) for a generic MCP client connecting to
// this server. Its URL (below) is the client's client_id, per
// draft-ietf-oauth-client-id-metadata-document.
app.get("/client-metadata.json", (c) => {
    const { MCP_SERVER_URL } = c.env;
    const clientId = `${MCP_SERVER_URL}/client-metadata.json`;

    return c.json({
        client_id: clientId,
        client_name: "Hono MCP Server",
        redirect_uris: [
            "https://hono-mcp-worker.abbaspour.workers.dev/callback",
